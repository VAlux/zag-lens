...
| `stale_ttl_seconds` | `300` | `0` through `604800`, in addition to `stale_after_seconds`; `0` never displays `stale`. |
...
Setting `stale_ttl_seconds "604800"` effectively restores the previous
indefinite behavior, where a stale status stays pinned until a new event
arrives or the pane closes.
